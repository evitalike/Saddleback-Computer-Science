...
int main()
{
	...
	// ... // [1.2] variable q of type pointer to double is points to x
		// Error. double pointer cannot point to an int variable.
	...
	*p = 5;
	...
	p = &x2;
	// p and p2 now points to x2

	// complete the following reference examples
	// indicate if the requested operation is not allowed, why not?
	// Q#1 - reference examples
	int y = 50;
	// ... // [1.8] variable r of type reference to int refers to x (i.e. int& r = ?;), nothing special to do here in the initializer
	int &r = x;
	// ... // [1.9] variable s of type reference to double is refers to x
		// Error. double reference cannot refer to an int variable.
	// ... // [1.10] update the value of r to 10, assign to y through r (notice * is not needed)
	r = 10;
	y = r;
	// ... // [1.11] store the value of r in variable y2 of type int, read r through y (notice * is not needed)
	int y2 = y;
	...
	// ... // [1.15] variable r3 of type reference to int is defined but not initialized (i.e. does not refer to an int)
		// Error. You cannot create an unitialized reference.
	...
	*pd = 3;
	...
	//pd += 1000;    // [2.3] // ... error explain
		// Error. This would cause pointer pd to go out of bounds of the array it points to
		// which could cause a segmentation fault.
	//double d = *pd;// [2.4] // ... error explain
		// This line compiles and does not cause any error currently, but has the potential
		// to cause a segmentation fault if pd pointed to uninitialized data.
	//*pd = 12.34;   // [2.5] // ... error explain
		// *pd is a double value, but in this line we are assigning a float to it 
	...
	//int dif2 = pd + pd2; // [2.7] // ... error explain
		// Error. pd and pd2 are both memory locations stored in hex, which cannot be stored within an int variable.
	//int dif3 = pd2 - pd3;// [2.8] // ... error explain
		// pd2 and pd3 point to two different arrays, meaning it would not make much sense to find the difference between them.
	...
	//if (pd6 != pd7) cout << "pointers point to the different elements of the array" << endl;
		// This logic would be incorrect, as pd6 and pd7 both use different arrays and will never have the same elements as each other.
	...
	// Q#4 - new, delete operator examples
	{
		...
		// the line above allocate *pi to an int.
		int* qi = new int[5];                           // [4.2] allocate five ints (an array of 5 ints)
		// the line above allocate *qi with an array of 5 ints
		int& ri = *pi;
		int& ri2 = *qi;
		int*& ri3 = qi;
		ri = 100;
		...
		// the line above allocates *pd to a double variable
		double* qd = new double[hw08::DYNAMIC_SIZE];    // [4.4] allocate DYNAMIC_SIZE doubles (an array of DYNAMIC_SIZE doubles)
		// the line above allocates  *qd to an array of size DYNAMIC_SIZE

		//pi = pd;  // [4.5] pi points to pd
					// ... error explain
			// a double pointer cannot be assigned to an int pointer
		//pd = pi;  // [4.6] pd points to pi
					// ... error explain
		// an int pointer cannot be assigned to a pointer of type double
		...
		delete pi;            // [4.7] how are the values of ri, ri2, ri3 affected by delete statement?
			// this would cause a memory leak, as pi refers to a pointer, and deleting
			// pi would delete only the address that it's pointer points too, not the array.
		delete[] ri3;        // [4.8] how are the values of ri, ri2, ri3 affected by delete statement?
			// this would delete the array that ri3 points to;
...
// Q#1: Describe what a pointer is. Describe what a reference is. Whatfs the difference between a pointer and a reference?
/* Q#1: Describe what a pointer is:
A pointer is a variable that holds the value of the memory address of another variable.
Pointers are declared using the * operator, and must point to a datatype of the same type as the pointer.
/
Q#1: Describe what a reference is:
A reference is an alternative name for a value of an existing variable, similar to using a const pointer.
A variable can be declared as reference by putting e&f in the declaration.
/
Q#1: What is the difference between a pointer and a reference:
A pointer is a data type that stores an address of a memory location
while A reference stores an address of a variable like a pointer, a
reference acts like an existing variable and when used in code. When you use arithmatic for a reference
you are modifying the variable that the reference stores.
use arithmatic for a pointer, you are changing the memory location that the pointer
points to. Both allow you to pass data to a function directly without copying the data.
/
Q#3: How is the twoDDoubles 2d array laid out in memory?
The the twoDDoubles 2d array is laid out in memory as a row of memory addresses one after another in order.
Q#3: Why is it necessary to cast twoDDoubles to a double* in the print_2darray_pointer function call?
it's necessary to cast twoDDoubles because you must dereference it within the function,
as you cannot acess it directly by using the array values for the rows and columns.
instead, you must use pointer arithmatic.
/
/
 Q#5: How is the p_p_tictactoe 2d dynamic array laid out in memory (i.e. why is p_p_tictactoe an int**)?
 the p_p_tictactoe array is laid out in memory as an array of int pointers, where each pointer points
 to an seperate array. The pointers are in order in memory, but the arrays that they point to are in various locations within memory.

 Q#5: Why do we need to pass a pointer to a pointer of type int in the print_2darray_dynamic_pointer function call?
 we have to pass a pointer to a pointer in the function because the array we are pass in is an 
 array of pointers, so we must do this in order to increment properly through each array by dereferencing.
*/